    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}">
        ...
        <div class="flex gap-2 items-center justify-between sm:hidden">
            ...
            <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-slate-400 bg-slate-50 border border-gray-200 cursor-not-allowed leading-5 rounded-l-md" aria-hidden="true">
        ...
        @else
            <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-gray-200 rounded-l-md leading-5 hover:bg-slate-50 hover:text-primary focus:z-10 focus:outline-none focus:ring ring-primary/20 focus:border-primary active:bg-slate-100 transition ease-in-out duration-150" aria-label="{{ __('pagination.previous') }}">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                </svg>
            </a>
        @endif
            ...
            <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-slate-700 bg-white border border-gray-200 rounded-r-md leading-5 hover:bg-slate-50 hover:text-primary focus:z-10 focus:outline-none focus:ring ring-primary/20 focus:border-primary active:bg-slate-100 transition ease-in-out duration-150" aria-label="{{ __('pagination.next') }}">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
        ...
        @else
            <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-slate-400 bg-slate-50 border border-gray-200 cursor-not-allowed leading-5 rounded-r-md" aria-hidden="true">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                    </svg>
                </span>
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                ...
                                @else
                                    <a href="{{ $url }}"
                                        class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-slate-700 bg-white border border-gray-300 leading-5 hover:text-slate-500 focus:z-10 focus:outline-none focus:ring ring-primary/20 focus:border-primary active:bg-gray-100 active:text-slate-700 transition ease-in-out duration-150"
